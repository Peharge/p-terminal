...
/// Führt externe Befehle synchron aus, immer via cmd.exe unter Windows.
fn spawn_shell_command(line: &str) {
    #[cfg(windows)]
    {
        // Immer cmd.exe verwenden
        ...
        match rl.readline(&prompt) {
            Ok(line) => {
                ...
                match parse_builtin(trimmed) {
                    ...
                    Builtin::Other => {
                        // Führe externe Befehle immer via cmd.exe (Windows) bzw. sh (Unix) aus
