...
# === Configuration ===
REQUIRED_PACKAGES = [
    "cirq-core[contrib]"  # Includes optional contrib features
]
...
def ensure_packages(packages: list[str]) -> None:
    ...
    missing = [pkg for pkg in packages if importlib.util.find_spec(pkg.split('[')[0]) is None]

    if not missing:
        logging.info("[PASS] ✅ All required packages are already installed.")
        return

    logging.info(f"[INFO] Missing packages: {', '.join(missing)}")
    for index, pkg in enumerate(missing, 1):
        print("")
        logging.info(f"[INFO] [{index}/{len(missing)}] Installing: {pkg}")
        try:
            ...
            if result.returncode == 0:
                logging.info(result.stdout)
                logging.info(f"[PASS] ✅ Successfully installed: {pkg}")
            else:
                logging.error(f"[ERROR] ❌ Failed to install {pkg}:\n{result.stderr}")

        except Exception as e:
            logging.error(f"[ERROR] ❌ Exception during installation of {pkg}: {e}")
...
def main():
    ...
    logging.info("[INFO] Checking and installing required packages...")
    ensure_packages(REQUIRED_PACKAGES)

    logging.info("[SUCCESS] ✅ Environment setup complete.")
