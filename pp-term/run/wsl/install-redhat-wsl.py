...
"""
This script installs Red Hat Enterprise Linux (RHEL) for WSL on Windows.
It performs the following steps:
  ...
  4. Starts the installation of the RHEL distribution (via wsl --install -d RedHatWSL).

Important:
  - The script MUST be run as Administrator.
  - A system restart may be required for the enabled features to take effect.
  - After installation, the first RHEL session will start in a new window where you will need to set up (username, password).

Note:
  - RHEL is not officially available in the Microsoft Store as a WSL distribution.
  - This script assumes that a custom WSL distribution named "RedHatWSL" has already been imported using `wsl --import`.
...
def install_redhat():
    """Starts the installation process for the RHEL distribution via WSL."""
    run_command(["wsl", "--install", "-d", "RedHatWSL"])
...
def main():
    ...
    logging.info("Starting Red Hat Enterprise Linux installation for WSL...")
    ...
    try:
        ...
        logging.info("The RHEL installation has started. A restart may be required for all changes to take effect.")
        logging.info("Upon the first start of RHEL, you will be prompted to create a username and password.")
    except Exception as e:
