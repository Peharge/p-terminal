...
"""
This script installs Ubuntu for WSL on Windows.
It performs the following steps:
  ...
  4. Starts the installation of the Ubuntu distribution (via wsl --install -d Ubuntu).
...
def install_ubuntu():
    """
    Starts the installation process for the Ubuntu distribution via WSL.
    """
    run_command(["wsl", "--install", "-d", "Ubuntu"])
...
def main():
    ...
    logging.info("Starting Ubuntu installation for WSL...")
    ...
    try:
        ...
        logging.info(
            "The Ubuntu installation has started. A restart may be required for all changes to take effect.")
        logging.info(
            "Upon the first start of Ubuntu, you will be prompted to create a username and password.")
    except Exception as e:
