...
"""
This script installs Fedora for WSL on Windows.
It performs the following steps:
  ...
  4. Starts the installation of the Fedora distribution (via wsl --install -d Fedora).

Important:
  - The script MUST be run as Administrator.
  - A system restart may be required for the enabled features to take effect.
  - After installation, the first Fedora session will start in a new window where you will need to set up (username, password).

Note:
  - Fedora is available as an official WSL distribution via the Microsoft Store.
...
def install_fedora():
    """Starts the installation process for the Fedora distribution via WSL."""
    run_command(["wsl", "--install", "-d", "Fedora"])
...
def main():
    ...
    logging.info("Starting Fedora installation for WSL...")
    ...
    try:
        ...
        logging.info("The Fedora installation has started. A restart may be required for all changes to take effect.")
        logging.info("Upon the first start of Fedora, you will be prompted to create a username and password.")
    except Exception as e:
