...
print("""
WARNING: This script is intended for installing or maintaining SUSE Linux on WSL.

This script is unofficial and may pose security risks.
Use at your own risk!

Do you really want to proceed with WSL SUSE installation/maintenance? [y/n]:
""", end='')
...
# Hier kann der Installations- oder Wartungscode für WSL SUSE folgen
print(f"[{timestamp()}] [INFO] Proceeding with WSL SUSE installation/maintenance...")
...
def install_suse():
    """
    Starts the installation process for the SUSE distribution via WSL.
    """
    run_command(["wsl", "--install", "-d", "openSUSE-Leap-15.4"])
...
def main():
    ...
    logging.info("Starting openSUSE Leap installation for WSL...")
    ...
    try:
        ...
        # Start the openSUSE Leap installation
        install_suse()

        logging.info(
            "The SUSE installation has started. A restart may be required for all changes to take effect.")
        logging.info(
            "Upon the first start of SUSE, you will be prompted to create a username and password.")
    except Exception as e:
