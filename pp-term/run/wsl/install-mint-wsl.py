...
"""
This script installs Linux Mint for WSL on Windows.
It performs the following steps:
  ...
  4. Starts the installation of the Linux Mint distribution (via wsl --install -d LinuxMint).

Important:
  - The script MUST be run as Administrator.
  - A system restart may be required for the enabled features to take effect.
  - After installation, the first Linux Mint session will start in a new window where you will need to set up (username, password).

Note:
  - Linux Mint is **not officially available** in the Microsoft Store as a WSL distribution.
    For this to work, a custom Linux Mint WSL distribution must be imported manually beforehand
    OR made available via a third-party WSL launcher (e.g. WSL-DistroLauncher).
...
def install_linux_mint():
    """Starts the installation process for the Linux Mint distribution via WSL."""
    run_command(["wsl", "--install", "-d", "LinuxMint"])
...
def main():
    ...
    logging.info("Starting Linux Mint installation for WSL...")
    ...
    try:
        ...
        logging.info("The Linux Mint installation has started. A restart may be required for all changes to take effect.")
        logging.info("Upon the first start of Linux Mint, you will be prompted to create a username and password.")
    except Exception as e:
