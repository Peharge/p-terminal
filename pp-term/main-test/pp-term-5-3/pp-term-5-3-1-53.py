...
def handle_special_commands(user_input):
    ...
    if user_input.startswith("pd-v "):
        ...
        if not user_input.endswith(".v"):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.v`-Datei an.")
            return True
        ...
        if which("v") is None:
            print(f"{timestamp()} [ERROR] `v`-Compiler nicht gefunden. Bitte installiere V: https://vlang.io")
            return True
        if which("gdb") is None:
            print(
                f"{timestamp()} [ERROR] `gdb` (GNU Debugger) nicht gefunden. Bitte installiere gdb (z. B. `sudo apt install gdb`).")
            return True

        # 3) Kompilieren mit Debug-Informationen
        print(f"{timestamp()} [INFO] Kompiliere {source_file} mit Debug-Info (v -g)...")
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Starten des Compilers: {e}", file=sys.stderr)
            return True

        if compile_proc.returncode != 0:
            print(f"{timestamp()} [ERROR] Kompilierung fehlgeschlagen:")
            print(compile_proc.stderr.strip())
            return True
        else:
            print(f"{timestamp()} [INFO] Kompilierung erfolgreich. Binärdatei: {executable}")

        # 4) Debuggen mit gdb
        print(f"{timestamp()} [INFO] Starte GNU Debugger (gdb) für {executable}...")
        ...
        try:
            ...
            print(f"{timestamp()} [INFO] Debugging durch Nutzer abgebrochen.")
        except subprocess.CalledProcessError as e:
            print(f"{timestamp()} [ERROR] Fehler beim Ausführen von gdb: {e}", file=sys.stderr)
    ...
    if user_input.startswith("pd-zig "):
        ...
        if not user_input.endswith(".zig"):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.zig`-Datei an.")
            return True
        ...
        if which("zig") is None:
            print(
                f"{timestamp()} [ERROR] `zig`-Compiler nicht gefunden. Bitte installiere Zig: https://ziglang.org/download/")
            return True
        if which("gdb") is None:
            print(
                f"{timestamp()} [ERROR] `gdb` (GNU Debugger) nicht gefunden. Bitte installiere gdb (z. B. `sudo apt install gdb`).")
        ...
        print(f"{timestamp()} [INFO] Kompiliere {source_file} mit Debug-Info (zig build-exe -g)...")
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Aufruf des Compilers: {e}", file=sys.stderr)
            return True
    ...
    if user_input.startswith("pd-nim "):
        ...
        if not user_input.endswith(".nim"):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.nim`-Datei an.")
            return True
        ...
        if which("nim") is None:
            print(
                f"{timestamp()} [ERROR] `nim`-Compiler nicht gefunden. Bitte installiere Nim: https://nim-lang.org/install.html")
            return True
        if which("gdb") is None:
            print(
                f"{timestamp()} [ERROR] `gdb` (GNU Debugger) nicht gefunden. Bitte installiere gdb (z. B. `sudo apt install gdb`).")
        ...
        # 3) Kompilieren mit Debug-Informationen
        print(f"{timestamp()} [INFO] Kompiliere {source_file} mit Debug-Info (nim c -d:debug --debugger:native)...")
        compile_cmd = [
            "nim", "c",
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Aufruf des Nim-Compilers: {e}", file=sys.stderr)
            return True
    ...
    if user_input.startswith("pd-carban "):
        ...
        if not user_input.endswith(".carban"):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.carban`-Datei an.")
            return True
        ...
        if which("carban") is None:
            print(f"{timestamp()} [ERROR] `carban`-Compiler nicht gefunden. "
                  f"Bitte installiere Carban von https://carban-lang.org/install oder aus deinem Paketmanager.")
            return True
        if which("gdb") is None:
            print(f"{timestamp()} [ERROR] `gdb` (GNU Debugger) nicht gefunden. "
                  f"Bitte installiere gdb (z. B. `sudo apt install gdb`).")
            return True

        # 3) Kompilieren mit Debug-Informationen
        print(f"{timestamp()} [INFO] Kompiliere {source_file} mit Debug-Info (carban compile -g)...")
        compile_cmd = [
            "carban", "compile",
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Aufruf des Carban-Compilers: {e}", file=sys.stderr)
            return True
    ...
    if user_input.startswith("pd-solidity "):
        ...
        if not user_input.endswith(".sol"):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.sol`-Datei an.")
            return True
        ...
        if which("solc") is None:
            print(
                f"{timestamp()} [ERROR] `solc` (Solidity-Compiler) nicht gefunden. "
                f"Bitte installiere Solidity (z. B. über `npm install -g solc` oder aus deinem Paketmanager)."
            )
            return True
        if which("evm") is None:
            print(
                f"{timestamp()} [ERROR] `evm` (EVM-Interpreter aus go-ethereum) nicht gefunden. "
                f"Bitte installiere go-ethereum (`geth`) oder das `evm`-Binary separat."
            )
            return True

        # 3) Kompilieren mit solc → JSON-Output mit Bytecode
        print(f"{timestamp()} [INFO] Kompiliere {source_file} (solc --combined-json bin)...")
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Aufruf von solc: {e}", file=sys.stderr)
            return True
        ...
        try:
            ...
            if not contracts_dict:
                print(f"{timestamp()} [ERROR] Konnte keine Contracts in der Ausgabe finden.")
                return True
            ...
            if not bytecode_hex:
                print(f"{timestamp()} [ERROR] Binärer Bytecode des Contracts ist leer.")
                return True
            # Prefix 0x, damit evm ihn richtig interpretiert
            bytecode = "0x" + bytecode_hex
        except json.JSONDecodeError as e:
            print(f"{timestamp()} [ERROR] JSON-Ausgabe von solc konnte nicht geparst werden: {e}", file=sys.stderr)
            return True

        print(f"{timestamp()} [INFO] Bytecode extrahiert (Länge: {len(bytecode_hex)} hex-Zeichen).")
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Konnte temporäre Hex-Datei nicht anlegen: {e}", file=sys.stderr)
            return True

        print(f"{timestamp()} [INFO] Bytecode in temporäre Datei geschrieben: {hex_file}")

        # 6) Debuggen mit EVM-Interpreter
        print(f"{timestamp()} [INFO] Starte EVM-Interpreter im Debug-Modus...")
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Ausführen von evm: {e}", file=sys.stderr)
        finally:
            # 7) Aufräumen: temporäre Datei löschen
            try:
                os.remove(hex_file)
                print(f"{timestamp()} [INFO] Entferne temporäre Datei: {hex_file}")
            except Exception:
    ...
    if user_input.startswith("pd-hack "):
        ...
        if not (filename.endswith(".hack") or filename.endswith(".php")):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.hack`- oder `.php`-Datei an, die Hack-Syntax verwendet.")
            return True
        ...
        if which("hh_client") is None:
            print(f"{timestamp()} [ERROR] `hh_client` (Hack-Typchecker) nicht gefunden. "
                  f"Bitte installiere `hh_single_type_check` bzw. HHVM mit `hh_client`. "
                  f"Siehe: https://docs.hhvm.com/hack/getting-started/getting-started")
            return True
        if which("hhvm") is None:
            print(f"{timestamp()} [ERROR] `hhvm` nicht gefunden. Bitte installiere HHVM: https://hhvm.com/")
            return True

        # 3) Typprüfung mit hh_client
        print(f"{timestamp()} [INFO] Starte Typprüfung von `{filename}` mit `hh_client check-file`…")
        try:
            ...
            print(f"{timestamp()} [ERROR] Konnte `hh_client` nicht starten: {e}", file=sys.stderr)
            return True

        # 3a) Rückgabewert überprüfen
        if proc.returncode != 0:
            print(f"{timestamp()} [ERROR] Typprüfung fehlgeschlagen. hh_client meldet:")
            # hh_client gibt sowohl auf stdout (JSON-Fehler) als auch stderr text aus – beides ausgeben
            if proc.stderr.strip():
                print(proc.stderr.strip())
            if proc.stdout.strip():
                print(proc.stdout.strip())
            print(f"{timestamp()} [INFO] Bitte behebe die oben aufgeführten Typfehler, bevor du fortfährst.")
            return True
        else:
            print(f"{timestamp()} [INFO] Typprüfung erfolgreich – keine Fehler in `{filename}` gefunden.")
        ...
        #
        print(f"{timestamp()} [INFO] Starte HHVM im Debug-Modus (DBGP) für `{filename}`…")
        hhvm_cmd = [
            "hhvm",
            "-c", "/dev/null",
            "-d", "hhvm.debugger.enable=1",
        ...
        try:
            ...
            print(f"{timestamp()} [INFO] HHVM läuft. Debugger lauscht auf Port 8080. "
                  f"Starte deine IDE mit DBGP (IDEKEY=VDEBUG).")
            dbg_proc.wait()
        except KeyboardInterrupt:
            print(f"{timestamp()} [INFO] Debugging durch Nutzer abgebrochen.")
        except subprocess.CalledProcessError as e:
            print(f"{timestamp()} [ERROR] Fehler beim Ausführen von HHVM: {e}", file=sys.stderr)
    ...
    if user_input.startswith("pd-crystal "):
        ...
        if not (filename.endswith(".cr") or filename.endswith(".crystal")):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.cr`- oder `.crystal`-Datei an.")
            return True
        ...
        if which("crystal") is None:
            print(
                f"{timestamp()} [ERROR] `crystal`-Compiler nicht gefunden. Bitte installiere Crystal: https://crystal-lang.org/install/")
            return True

        # 3) Kompilieren mit Debug-Info
        print(f"{timestamp()} [INFO] Kompiliere `{filename}` mit Debug-Informationen (`crystal build --debug`)…")
        compile_cmd = [
            "crystal",
            "build",
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Aufruf des Crystal-Compilers: {e}", file=sys.stderr)
            return True
        ...
        if which("gdb") is not None:
            ...
            print(f"{timestamp()} [ERROR] Weder `gdb` noch `lldb` gefunden. "
                  f"Bitte installiere einen nativen Debugger (z. B. `gdb` oder `lldb`).")
            return True
        ...
        if debugger == "gdb":
            ...
            print(f"{timestamp()} [INFO] Starte LLVM Debugger (lldb) für `{executable}`…")
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Ausführen des Debuggers ({debugger}): {e}", file=sys.stderr)
    ...
    if user_input.startswith("pd-haxe "):
        ...
        if not filename.endswith(".hx"):
            print(f"{timestamp()} [ERROR] Bitte gib eine `.hx`-Datei an.")
            return True
        ...
        if which("haxe") is None:
            print(
                f"{timestamp()} [ERROR] `haxe`-Compiler nicht gefunden. Bitte installiere Haxe: https://haxe.org/download/")
            return True
        ...
        if which("gdb") is not None:
            ...
            print(
                f"{timestamp()} [ERROR] Weder `gdb` noch `lldb` gefunden. Bitte installiere einen nativen Debugger (z. B. `gdb` oder `lldb`).")
            return True

        # 4) Kompilieren mit Debug-Informationen in C++
        print(f"{timestamp()} [INFO] Kompiliere `{filename}` mit Debug-Info (haxe → C++ via hxcpp)…")
        ...
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Aufruf des Haxe-Compilers: {e}", file=sys.stderr)
            return True

        if compile_proc.returncode != 0:
            print(f"{timestamp()} [ERROR] Haxe-Kompilierung fehlgeschlagen:")
            if compile_proc.stderr:
                print(compile_proc.stderr.strip())
            return True
        else:
            print(f"{timestamp()} [INFO] Haxe-Generierung erfolgreich. Wechsle nach `bin/` zum Builden…")

        # 5) In das Verzeichnis 'bin' wechseln und 'make' ausführen
        bin_dir = os.path.join(os.getcwd(), "bin")
        if not os.path.isdir(bin_dir):
            print(
                f"{timestamp()} [ERROR] Verzeichnis `bin/` nicht gefunden nach Haxe-Generierung. Ist hxcpp installiert und konfiguriert?")
            return True

        # Unter macOS kann das Executable später die Endung ".app" haben; wir gehen erst vom Standard aus.
        os.chdir(bin_dir)
        print(f"{timestamp()} [INFO] Führe `make` im Verzeichnis `bin/` aus, um das native Binary zu erstellen…")
        try:
            ...
            print(f"{timestamp()} [ERROR] Fehler beim Ausführen von `make`: {e}", file=sys.stderr)
            os.chdir("..")
            return True

        if make_proc.returncode != 0:
            print(f"{timestamp()} [ERROR] Build mit `make` fehlgeschlagen:")
            if make_proc.stderr:
                print(make_proc.stderr.strip())
            os.chdir("..")
            return True
        else:
            print(f"{timestamp()} [INFO] Build erfolgreich. Wechsel zurück ins Hauptverzeichnis…")
            os.chdir("..")
        ...
        if not os.path.isfile(executable_path):
            print(
                f"{timestamp()} [ERROR] Executable `{executable_path}` nicht gefunden. Build möglicherweise fehlgeschlagen.")
            return True

        # 7) Debuggen starten
        if debugger == "gdb":
            print(f"{timestamp()} [INFO] Starte GNU Debugger (gdb) für `{executable_path}`…")
            debug_cmd = ["gdb", "--args", executable_path]
        else:
            print(f"{timestamp()} [INFO] Starte LLVM Debugger (lldb) für `{executable_path}`…")
