...
{hyperlink("[GitHub Repository]", "https://github.com/Peharge/p-terminal")} {hyperlink("[Project Website]", "https://peharge.github.io/MAVIS-web/p-term.html")} {hyperlink("[Learn PP-Term]", "https://github.com/Peharge/p-terminal/blob/main/LEARN.md")}""")
...
def save_current_env(env_path: str | Path) -> None:
    try:
        ...
        print(f"[DEBUG] Saved active_env: {full_path}")
    except Exception as e:
        logging.error(f"Error saving status file: {e}")


def load_saved_env() -> Optional[str]:
    """Lädt den zuletzt gespeicherten Virtualenv-Pfad (als String), erstellt die Datei bei Bedarf."""
    try:
        ensure_state_dir_exists()
        if not STATE_FILE.exists():
            save_current_env("")  # Leeren Eintrag initial schreiben
            return None
