...
def save_current_env(env_path: str) -> None:
    ...
    try:
        ...
        logging.debug(f"Aktives Env gespeichert: {env_path}")
    except Exception as e:
        logging.error(f"Fehler beim Speichern des Statusfiles: {e}")
...
def load_saved_env() -> Optional[str]:
    ...
    try:
        if STATE_FILE.exists():
            ...
            logging.debug(f"Geladener gespeicherter Env: {value} ({type(value)})")
            if value and os.path.isdir(value):
                return value
    except Exception as e:
        logging.warning(f"Fehler beim Laden des Statusfiles: {e}")
    return None
...
def find_env_in_current_dir() -> Optional[str]:
    ...
    """
    cwd = os.getcwd()
    logging.debug(f"Suche nach Virtualenvs in: {cwd}")
    for item in os.listdir(cwd):
        ...
        if os.path.isdir(item_path) and os.path.isfile(activate_path):
            logging.info(f"Gefundene Virtualenv: {item_path}")
...
def find_active_env() -> str:
    ...
    if saved:
        logging.info(f"Verwende gespeichertes Virtualenv: {saved}")
        return saved

    logging.warning(f"Kein Virtualenv gefunden. Verwende Fallback: {DEFAULT_ENV_DIR}")
...
def main():
    ...
    while True:
        try:
            ...
            env_active = os.path.exists(DEFAULT_ENV_DIR)
