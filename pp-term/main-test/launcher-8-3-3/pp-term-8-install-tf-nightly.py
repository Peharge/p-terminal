...
def detect_cuda_available() -> bool:
    ...
    try:
        ...
        logging.info("[INFO] nvcc detected -> GPU-supported tf-nightly install")
        return True
    except Exception:
        logging.info("[INFO] nvcc not found -> CPU-only tf-nightly install")
        return False


def install_tf_nightly(gpu: bool) -> None:
    """
    Installs tf-nightly via pip3.
    Uses GPU-enabled package if CUDA is available, else CPU-only package.
    """
    pkg = 'tf-nightly' if gpu else 'tf-nightly-cpu'
    logging.info(f"[INFO] Installing TensorFlow nightly build: pip3 install {pkg}")
