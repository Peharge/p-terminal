...
def install_oh_my_posh():
    ...
    try:
        ...
        subprocess.run(["winget", "install", "atomic.OhMyPosh"], check=True)
...
def initialize_oh_my_posh():
    ...
    try:
        config_path = os.path.expanduser("~") + r"\AppData\Local\Programs\oh-my-posh\themes\atomic.omp.json"
...
def upgrade_oh_my_posh():
    ...
    try:
        ...
        subprocess.run(["winget", "upgrade", "atomic.OhMyPosh"], check=True)
