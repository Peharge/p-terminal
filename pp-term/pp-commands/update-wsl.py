...
def run_docker_update() -> int:
    """
    Pull latest versions of all local Docker images.

    Returns:
        int: 0 if all pulls succeeded, non-zero on first failure.
    """
...
def run_wsl_update() -> int:
    """
    Update WSL kernel and all installed distributions.

    Uses `wsl --update` to apply kernel updates, and iterates over distributions
    for in-place upgrades if supported.
    ...
    for distro in distros:
        # Upgrade distro if supported (requires Windows 11 and certain distro features)
