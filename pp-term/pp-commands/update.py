...
def activate_virtualenv(venv_path: Path):
    """Activates the virtual environment."""
    activate_script = venv_path / 'Scripts' / 'activate_this.py'
    if not activate_script.exists():
        print(f"[{timestamp()}] [ERROR] Activation script not found: {activate_script}")
        sys.exit(1)

    print(f"[{timestamp()}] [INFO] Activating virtual environment at: {venv_path}")
