...
def download_font(url, font_zip_path="FiraCode.zip"):
    ...
    try:
        ...
        print(f"[{timestamp()}] [INFO] Download complete.")
        ...
        for file in files:
            if file.endswith(".ttf"):
                ...
                try:
                    ...
                    print(f"[{timestamp()}] [ERROR] Installed font: {file}")
                except Exception as e:
                    print(f"[{timestamp()}] [ERROR] Failed to install font {file}: {e}")
                    continue

    # Clean up extracted files
    shutil.rmtree("FiraCode")

    print(f"[{timestamp()}] [INFO] Font installation complete.")
    ...
    if sys.platform == "win32":
        try:
            ...
            subprocess.run(["powershell", "-Command", "powershell -ExecutionPolicy Bypass -File install-winget.ps1"],
                           check=True)
            print(f"[{timestamp()}] [INFO] winget installed successfully.")
...
def install_oh_my_posh():
    ...
    try:
        ...
        subprocess.run(["winget", "install", "tokyo.OhMyPosh"], check=True)
        print(f"[{timestamp()}] [INFO] OhMyPosh installed successfully.")
    except subprocess.CalledProcessError as e:
...
def initialize_oh_my_posh():
    ...
    try:
        config_path = os.path.expanduser("~") + r"\AppData\Local\Programs\oh-my-posh\themes\tokyo.omp.json"
        command = f"oh-my-posh --init --shell pwsh --config {config_path} | Invoke-Expression"
        subprocess.run(["powershell", "-Command", command], check=True)
        print(f"[{timestamp()}] [INFO] OhMyPosh initialized successfully.")
    except subprocess.CalledProcessError as e:
...
def upgrade_oh_my_posh():
    ...
    try:
        ...
        subprocess.run(["winget", "upgrade", "tokyo.OhMyPosh"], check=True)
        print(f"[{timestamp()}] [INFO] OhMyPosh upgraded successfully.")
    except subprocess.CalledProcessError as e:
...
if __name__ == "__main__":
    ...
    print(f"[{timestamp()}] [INFO] Done! Fira Code Nerd Font is now installed and configured.")
    ...
    print(f"[{timestamp()}] [INFO] All steps completed successfully.")
