...
log_path = Path(__file__).parent / "installer_jax.log"
logging.basicConfig(
    level=logging.INFO,
...
def detect_cuda_version() -> str | None:
    """
    Detects installed CUDA version via nvcc.
    Returns string like 'cuda11_cudnn82' if supported, else None.
    """
    try:
        ...
        if m:
            ...
            tag = f"cuda{major}_cudnn82"
            logging.info(f"[INFO] Detected CUDA {major}.{minor} -> JAX GPU tag '{tag}'")
            return tag
    except Exception:
        logging.info("[INFO] nvcc not found or CUDA not installed -> CPU-only JAX install")
    return None


def install_jax(cuda_tag: str | None) -> None:
    """
    Installs JAX via pip3.
    If cuda_tag is provided, installs GPU-enabled JAX with appropriate extras and find-links URL.
    Otherwise installs CPU-only JAX.
    """
    if cuda_tag:
        pkg = f"jax[{cuda_tag}]"
        link = "https://storage.googleapis.com/jax-releases/jax_cuda_releases.html"
        cmd = ['pip3', 'install', '--upgrade', pkg, '-f', link]
        logging.info(f"[INFO] Installing GPU-enabled JAX: pip3 install --upgrade {pkg} -f {link}")
    else:
        cmd = ['pip3', 'install', '--upgrade', 'jax', 'jaxlib']
        logging.info("[INFO] Installing CPU-only JAX: pip3 install --upgrade jax jaxlib")

    logging.info(f"[INFO] Running: {' '.join(cmd)}")
    res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if res.returncode == 0:
        logging.info(f"[PASS] ✅ Installation succeeded:\n{res.stdout}")
    else:
        logging.error(f"[ERROR] ❌ Installation failed:\n{res.stderr}")
...
def main():
    ...
    logging.info(f"[INFO] [{timestamp()}] Starting JAX installation...")
    install_jax(cuda_tag)
